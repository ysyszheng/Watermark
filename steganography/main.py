...
def makeImageEven(image):
    ...
    evenPixels = [
        (r >> 1 << 1, g >> 1 << 1, b >> 1 << 1, t >> 1 << 1) for [r, g, b, t] in pixels
    ]
...
def decodeImage(image):
    ...
    endIndex = (
        locationDoubleNull + (8 - (locationDoubleNull % 8))
        if locationDoubleNull % 8 != 0
        else locationDoubleNull
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-m", "--message", type=str, help="steganography message", required=False
    )
